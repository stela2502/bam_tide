	...
	fn process_feature(
        ...
        if let Some(mate) = mate_read {
        	...
        	if let Some(processor) = mutations {
	             processor.handle_mutations( primary_read, "unknown", mut_idx, mut_gex, mapping_info, &cell_id);
	        }
        }
...
impl BedData {
	...
	fn collect_ghus( &self, read: &ReadData, ghus: &mut HashSet<GeneUmiHash>, idx: &mut IndexedGenes, ) {
		...
		let (spliced_read, _final_position) = SplicedRead::new( &read.cigar, read.start.try_into().unwrap() );
