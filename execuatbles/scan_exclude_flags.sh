...
for f in "${FLAGS[@]}"; do
  ...
  OUT=$(./run_compare.sh "$BAM" "$BIN_WIDTH" "--sam-flag-exclude $f" 2>&1)
  rc=$?

  if [[ $rc -ne 0 ]]; then
    printf "%-6s  ERROR (run_compare.sh exit %d)\n" "$f" "$rc"
    echo "./run_compare.sh $BAM $BIN_WIDTH --sam-flag-exclude $f \n$OUT"
    continue
  fi

  total_line=$(grep -E '^TOTAL[[:space:]]' "$REPORT" | tail -n 1 || true)
